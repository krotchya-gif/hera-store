...
import { Search, Package, ArrowLeft, Filter, Star, Grid, List, X, Heart } from 'lucide-react';
...
export default function SearchResultsPage({ query, setCurrentPage, setSelectedProduct, addToCart }) {
  ...
  useEffect(() => {
    setPage(1);
  }, [query, sortBy, priceRange.min, priceRange.max, selectedRating.length, selectedCategory]);
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      try {
        ...
        addToast('Gagal memuat hasil pencarian', 'error');
      } finally {
        setLoading(false);
      }
    };
  ...
  }, [query, page, sortBy, priceRange.min, priceRange.max, selectedRating, selectedCategory]);
        ...
        <div className="flex-1">
          ...
          {loading ? (
            ...
            <EmptyState
              ...
              description={`Tidak ada produk yang cocok dengan "${query}" dan filter yang dipilih.`}
              action={
                <button
                  onClick={() => setCurrentPage('home')}
